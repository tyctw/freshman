...
import { BookOpen, Map, Compass, CheckCircle, Menu, X, GitCompare } from 'lucide-react';
import { useState } from 'react';

const navItems = [
  { name: '首頁', href: '#hero', icon: Compass },
  { name: '高中 VS 高職', href: '#comparison', icon: GitCompare },
  { name: '學習指南', href: '#academics', icon: BookOpen },
  { name: '校園生活', href: '#life', icon: Map },
  { name: '暑期清單', href: '#checklist', icon: CheckCircle },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-6">
            <div className="flex space-x-6 font-bold text-sm uppercase tracking-wide">
            ...
            <a href="#" className="bg-white border-2 border-[#1E293B] text-[#1E293B] px-4 py-2 rounded-full font-bold text-sm shadow-[4px_4px_0px_0px_#1E293B] hover:translate-y-0.5 active:shadow-none transition-all flex items-center gap-2">
              <span>回落點分析</span>
            </a>
          </div>
          <div className="md:hidden flex items-center gap-3">
             <a href="#" className="bg-[#1E293B] text-white px-3 py-1.5 rounded-full text-xs font-bold shadow-[2px_2px_0px_0px_#FBBF24] active:shadow-none active:translate-y-[2px] transition-all">
               回落點分析
             </a>
             <button 
               onClick={() => setIsOpen(!isOpen)}
               className="p-1.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F59E0B] bg-slate-100 rounded-lg text-slate-700"
               aria-expanded={isOpen}
               aria-controls="mobile-menu"
               aria-label={isOpen ? "關閉選單" : "開啟選單"}
             >
               {isOpen ? <X className="w-6 h-6 stroke-[3]" aria-hidden="true" /> : <Menu className="w-6 h-6 stroke-[3]" aria-hidden="true" />}
             </button>
          </div>
        </div>
      </div>
        ...
        {isOpen && (
          <>
            <motion.div
              ...
              className="fixed inset-0 bg-[#1E293B]/40 backdrop-blur-sm z-[55] md:hidden h-screen w-screen"
              aria-hidden="true"
              style={{ top: 0, left: 0 }}
            />
            <motion.div
              ...
              className="fixed inset-y-0 right-0 w-[280px] sm:w-[320px] bg-white border-l-4 border-[#1E293B] shadow-[-8px_0_0_0_rgba(30,41,59,0.1)] z-[60] md:hidden flex flex-col h-screen"
            >
